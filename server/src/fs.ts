...
class InMemoryDatabase implements FsDatabase {
    ...
    async getFile(id: string): Promise<FileInfo> {
        const file: FileInfo | null = InMemoryDatabase.firstOrDefault(this.files.filter(f => f.id == id));
        if (file === null)
    ...
    async putFile(path: string): Promise<FileInfo> {
        if (InMemoryDatabase.firstOrDefault(this.files.filter(f => f.path == path)))
...
export class FsUtils {
    ...
    private static get dir() { return path.resolve(process.env.FS_FOLDER); }
    ...
    public static async getFile(id: string): Promise<[ReadStream, number]> {
        ...
        if (!await FsUtils.exists(_path))
            throw `File "${_path}" does not exist!`

        var stat = await fs.stat(_path);
        return [fsSync.createReadStream(_path, { flags: "r", encoding: "binary" }), stat.size]
    }

    public static async putFile(data: Readable): Promise<FileInfo> {
        const file = await FsUtils.db.putFile(pathids.rnd());
        var str = fsSync.createWriteStream(path.join(FsUtils.dir, file.path), { flags: "w", encoding: "binary" })
